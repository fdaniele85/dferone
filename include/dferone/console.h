...
namespace dferone::console {
     ...
     */
    enum class Colour { Red = 31, Green = 32, Yellow = 33, Blue = 34, Magenta = 35, Default = 39 };
     ...
     */
    constexpr typename std::underlying_type<Colour>::type colour_code(const Colour &c) { return static_cast<typename std::underlying_type<Colour>::type>(c); }
     ...
     */
#ifndef AVOID_COLORS
    inline std::string escape(Colour c) { return "\033[" + std::to_string(colour_code(c)) + "m"; }
#else
    inline std::string escape(Colour c) { return ""; }
#endif
     ...
     */
    inline std::ostream &operator<<(std::ostream &out, const Colour &c) { return out << escape(c); }
