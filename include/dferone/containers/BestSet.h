...
namespace dferone::containers {
    ...
    template<typename T, typename comparator = std::greater<T>>
        requires requires(T el, T el2, comparator comp) { comp(el, el2); }
    class BestSet {
    public:
        using value_type = T;
        ...
        inline const value_type &top() const { return elements_[0]; }
        ...
        inline const_iterator cend() const { return cbegin() + size_; }
